...
- **「测试并获取模型」**：保存前不落盘连通测试，直接拉取上游模型目录预览；编辑时 key 回填
  （列表接口向已鉴权面板回传 key，免每次测试重新粘贴）。
...
- **数据清理与自定义模型解耦**：设置页「数据清理」任何级别不再清掉自定义模型源（三级"删除配置"
  保留 providers/routing 写回）；自定义模型页新增独立「清空全部」按钮（二次确认，连同目录缓存清理）。
- **出站链路文档**：ROUTING.md 新增"谁走 sing-box 谁直连"专节（Tier 语义 / 各进程池来源 /
  E1 接管 / 网关配置多写者警示与丢段恢复法）。
