...
| **E1** | 上游代理出口（socks5 直连 Clash，MVP） | ⬜ 未开工 | — |
| **E2** | Clash selector 切换出口 IP 分散 | 🕓 延后 | — |
...
**功能开发**：
1. 坏池分类型恢复（2026-08-14 用户拍板）：链路类（连接失败/超时/5xx）自动恢复——
   `badUntil = now + bad_pool_reset_sec`(300)，到期放行 1 探测，成功清/失败重坏；
   **账号类（401/402/429）不自动恢复**（保持永久禁用，用户手动处理，UI 一键清除入口）。
...
## 四·五、E 系列：代理出口模式（2026-08-14 并入，源自群友建议文档）

> 来源：`docs/群友建议_EGRESS-PROXY-MODE.md`（2026-08-10 验证记录，已核实未实施）。
> **痛点**：本机裸连出口 IP 被 OpenCode 上游风控/限流 → 批量扫描 0/55 全挂、实例启动失败；
> 单节点低频测试却通过（2/55）。经 Clash 代理出口访问上游返回 200（已验证）。

### E1：上游代理出口（MVP，推荐先做）

**目标**：设置页填一个「上游代理」（如 `socks5://127.0.0.1:7897` 或 Clash mixed-port），
所有实例的 `active_socks5` 直接指向它（跳过 sing-box 出口），绕过本机裸连 IP 限流。

**功能开发**（文件级）：
1. 配置项 `upstream_proxy`（string，默认 "" = 现状直连）：`types_config.go` + `core/manager/config.go` 解析 + `config.example.json`。
2. `core/manager/opencodecfg.go` 生成逻辑：`upstream_proxy` 非空时 `active_socks5` / `socks5_proxies` 指向上游代理（解析 scheme：`socks5://` / `http://` 去前缀取 host:port；Clash mixed-port 同时支持两者，统一存 host:port）。
3. 设置页「代理出口」区块（`src/pages/SettingsPage.tsx`）：输入框（placeholder `socks5://127.0.0.1:7897`，留空 = 直连）+ 说明文案。
4. 扫描/实例/网关全自动受益：探针与实例配置生成走同一 `buildOpenCodeCfg`，无需额外改动。
5. 注意：配置后需重启实例/网关生效（配置热更新对 `active_socks5` 语义待确认，先按重启实例生效实现）。

**测试**：
- 配置解析：`upstream_proxy` 空/带 scheme/带端口非法值回退
- `opencodecfg` 生成：配置后 active_socks5 指向代理地址 + socks5_proxies 单元素；未配置 = 现状（回归快照）
- 前端 build + 设置页渲染
...
### E2：Clash selector 出口分散（进阶，延后）

- 经 Clash 外部控制 API（`PUT /proxies/CF-Selector {"name":"CF-Opt-01"}`）在每实例启动前切换 selector 到不同节点 → 出口 IP 分散，多实例并行不共享单一出口。
- 依赖 E1 的代理出口通道；需轮询切换 + 失败重试。
- **验收**：N 实例并行时出口 IP 分散（N≥2），不互相干扰。
...
| 8b | E1 | 上游代理出口配置生效，扫描/实例走代理出口 | ⬜ | 填 Clash 代理后批量扫描不再全挂 |
