...
### 阶段 P1：链路级主动探活 + 质量评分模型（core 侧）（✅ 已完成，2026-08-12，提交 d594468）
...
### 阶段 P2：加权路由 + 熔断/半开自动恢复（core 侧）（✅ 已完成，2026-08-12，提交 b848ce4）

**目标**：路由层消费质量分，坏节点自动降权剔除、恢复自动回归。

#### 阶段开头：P1 遗留声明
- 无（P1 全部验收通过：探活间隔/窗口/超时可配、质量分反映"断断续续"场景、无探活风暴）。
...
### 阶段 P3：UI 可视化 + 全链路收尾（前端）（✅ 已完成，2026-08-12，提交 2a5403b；节点池页质量展示 = 实例级数据已覆盖，不重复实现）

**目标**：六页 UI 中暴露质量分与熔断状态，设置页可调参数，完成 P 系列交付。

#### 阶段开头：P2 遗留声明
- 无（P2 全部验收通过：熔断阈值/半开可配、切换事件日志可见、性能模式开关生效）。
...
### 附加整改：P2b 请求级竞速 + D 系列（海鸥复核驱动，2026-08-12）

> 复核清单见 `docs/REVIEW-PD-DIFF.md`；本轮全部补齐并推送（`go test -count=1 ./...` 9 包全绿）：
>
> - **P2b 请求级竞速**（`64d2a20`）：`pool_race_copies`（默认 2）+ 一次请求并行扇出多候选，
>   非流式首个完整 2xx 胜出、流式首个 chunk 锁流胜出，其余 `cancel()`；配置全链路可配。
> - **D1 退出二次确认**（`b0bf9ba`）：设置页「退出程序」→ 三选弹窗（退出并释放 / 不释放 / 取消），
>   释放按 4 并发 + 全局进度面板，完成调壳退出。
> - **D2 一键测试三分类**（`782cad3`）：仅测 Running 实例，未启动计「跳过」，不再误报失败。
> - **D3 并发设置抽离**（`820576c`）：scan/batch/test/pool_probe 并发可配 + 设置页「并发设置」分组。
