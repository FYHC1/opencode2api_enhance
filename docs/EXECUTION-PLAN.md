...
第五阶段（P 系列，2026-08-16 新增，真实机验收反馈，需求见 REQ-P-SERIES.md）：
  P-1 订阅过滤与 Clash 对齐（P1）→ P-2 订阅导入边界收敛为节点池 + 删除同步清理（P3b+P4）
  → P-3 新增订阅立即导入 + loading/10s 看门狗（P2）→ P-4 前端全局扫描进度 + 停止保留结果 + 按钮 loading 审计（P3a+P5）
```
...
| 8 | **CONC-1** | 请求取消穿透（H1）+ 429 退避感知 ctx（L3） | ✅ 已完成（`d120ad6`） | 🔴 热路径正确性 |
| 9 | **CONC-2** | http.Client 按 addr 缓存（H3）+ 模型目录 generation（M8） | ✅ 已完成（`20e70ae`） | 独立 |
| 10 | **CONC-3** | 统计/调用日志写盘合并（H2 + M4 写侧） | ✅ 已完成（`ae22129`） | 独立 |
| 11 | **CONC-4** | Stop 短锁化（H4）+ 订阅端口记账（H6）+ 订阅缓存互斥（M7） | ✅ 已完成（`cd29df7`） | 独立 |
| 12 | **CONC-5** | 网关启动互斥（M1）+ 密钥快照（M2） | ✅ 已完成（`43f1c9c`） | 独立 |
| 13 | **CONC-6** | windsurf 账号借用互斥（H5/L4）+ midstream 锁内网络 IO（M1'） | ✅ 已完成（`df1c01f`，midstream 采用锁外网络读 + commit 分段重构） | 🔴 风险高 |
| 14 | **CONC-7** | 聚合/健康/订阅并行（M3 + M5 + M6） | ✅ 已完成（`8be36ef`） | 独立 |
| 15 | **CONC-8** | 日志读侧尾部 + 管理端统计并行（M4 读侧 + L9） | ✅ 已完成（`4b95e17`） | 独立 |
| 16 | **CONC-9** | 前端：轮询守卫（M9）+ App 重渲染（M10）+ 扫描状态机（M11）+ L10/杂项 | ✅ 已完成（`9aff256`） | 依赖后端无 |
| 17 | **CONC-10** | 后端低危批（L1/L2/L5/L6/L7/L8） | ✅ 已完成（`3987449`） | 独立 |
| 18 | **P-1** | 订阅过滤与 Clash 对齐（P1） | ✅ 已完成（`39927e6`） | 独立 |
| 19 | **P-2** | 订阅导入只进节点池 + 删除同步清理（P3b 边界 + P4）+ 文档 | ✅ 已完成（`dcf05bd`） | 独立 |
| 20 | **P-3** | 新增订阅保存+立即导入节点池 + loading/10s 看门狗/进度（P2） | ✅ 已完成（`3bdd568`） | 依赖 P-2 |
| 21 | **P-4** | 前端全局扫描进度（P3a）+ 停止保留结果（P5）+ 按钮 loading 审计（P2 audit） | ✅ 已完成（`78da4e6`） | 独立 |
...
## 第五阶段（P 系列）：真实机验收反馈修复（2026-08-16 新增）

**背景**：用户真实机验收（便携包 v1.3.4）反馈 5 项，需求与根因见 `docs/REQ-P-SERIES.md`。
已拍板设计决策：①订阅导入只进节点池（移除入池/独享目标，实例手动勾选添加）；②导入 10s 看门狗超时、后台不打断；
③停止扫描保留已扫结果。

### 阶段 P-1：订阅过滤与 Clash 对齐（P1）
1. `isInfoPseudoNode` 增强：全角冒号归一为半角（对齐 `isJunkNode`），或组合判定 `isInfoPseudoNode || isJunkNode`（clash_parse.go 同包）。
2. 两个过滤点：`parseSubscription`（subscribe.go:221）与 `parsePlainLinks`（:343）。
3. 存量旧缓存脏数据：`listSubscriptionNodes` 展示路径套同一过滤，立即隐藏历史伪节点。
- 测试：半角/全角/无冒号关键词表驱动 + 解析集成（伪节点不入缓存）+ 存量展示过滤。全绿后提交 `fix(P-1): ...`。

### 阶段 P-2：订阅导入只进节点池 + 删除同步清理 + 文档（P3b 边界 + P4）
1. 后端：`ImportSubscriptionNow` / `importSubscriptionForSource` 一律走 `importSubscriptionPool`（拉取+缓存，不建实例）；
   `SubscriptionTarget` 判定收敛为节点池语义（保留类型兼容旧配置，导入全按节点池处理）。
2. `SubscriptionsDeleteHandler`：删除源后同步清理订阅缓存中该分组节点（`RemoveSubscriptionNodes`），节点池页不再残留。
3. 文档：`CONFIGURATION.md`（或 API.md）与 `REQ-P-SERIES.md` 写入边界「订阅导入只更新节点池；实例由节点池手动添加」。
- 测试：导入不再创建实例（target=solo 旧配置兼容）、删除订阅清缓存分组、订阅测试回归。全绿后提交 `fix(P-2): ...`。

### 阶段 P-3：新增订阅保存 + 立即导入节点池 + loading/10s 看门狗（P2）
1. 后端：`SubscriptionsAddHandler` 保存源后同步执行一次 `ImportSubscriptionNow`（节点池导入），响应返回 `{imported, target}`。
2. 前端（NodesPage 订阅弹窗）：确定按钮 loading（「导入中…」+ 进度条）→ 10s 看门狗超时报错（「拉取超时，可稍后重试」），
   按钮恢复可用；后台请求不打断（节点稍后出现，刷新可见）。完成显示「已导入 N 个节点」后关闭弹窗并刷新节点池。
3. 弹窗文案与 UI：移除「导入目标：入池/独享」选择（P-2 已做后端，此处同步删前端控件）。
- 测试：后端 add+import 单测（httptest 假订阅）；前端 build + 走查（10s 超时路径、错误 toast）。提交 `fix(P-3): ...`。

### 阶段 P-4：前端全局扫描进度 + 停止保留结果 + 按钮 loading 审计（P3a + P5 + P2 audit）
1. App 全局扫描进度：有 busy scan/stop-scan 任务时，App 侧每 ~2s 轮询 `/scan/status` 并 upsert 任务进度
   （节点页卸载后悬浮窗进度继续走；兜底 stale 检查保留为最后防线）。
2. 停止保留结果（P5）：NodesPage 停止时不清空 `scan.results`（仅隐藏进度条）；停止完成后用后端最终
   `scanStatus` 回填结果——已扫节点的「成功：5643ms」等徽章保留，可继续勾选可用节点。
3. 按钮 loading 审计：全面检查六页所有异步按钮（提交/导入/删除/释放/重启/重置/清空日志/退出等），
   无 busy 态的一律补上（spinner + 禁用），对齐既有 `subBusy/importingUrl/acting` 模式。
- 测试：前端 build + 走查（切页悬浮窗续走、停止后结果保留、各异步按钮 loading）。提交 `fix(P-4): ...`。
...
| 25 | P-1 | 订阅导入伪节点过滤与 Clash 一致（官网/时间不再出现） | ⬜ | | 表驱动单测 + 真实订阅导入观察 |
| 26 | P-2 | 订阅导入只进节点池；删除订阅同步清节点池；文档边界明确 | ✅ | 2026-08-16 | 导入不建实例单测 + 删除订阅实测 |
| 27 | P-3 | 确定=保存+立即导入；按钮 loading；10s 超时报错不无限转圈 | ✅ | 2026-08-16 | 假订阅单测 + 浏览器走查 |
| 28 | P-4 | 切页悬浮窗进度续走；停止扫描保留结果徽章；异步按钮均有 loading | ✅ | 2026-08-16 | 走查 + 真实扫描/停止实测 |
