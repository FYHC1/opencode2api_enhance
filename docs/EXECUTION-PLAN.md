...
| 1 | **U1** | 节点池 poll 自杀 bug 修复（缺陷） | ⬜ 待派工 | 🔴 优先 |
| 2 | **U2** | 实例池状态筛选（REQ-POOL-FILTER） | ⬜ 待派工 | 独立 |
| 3 | **U3** | 轮询配置 `ui_poll_interval_sec` + 独享轮询 + 折叠面板 | ⬜ 待派工 | 独立 |
| 4 | **U4** | 日志页分页（每页 100） | ⬜ 待派工 | 独立 |
| 5 | **U5** | 统计页纯 CSS 迷你图 | ⬜ 待派工 | 独立 |
...
| 8 | U1 | poll 自杀 bug 修复：停止后再扫描进度正常 | ⬜ | | 复现路径浏览器走查 |
| 9 | U2 | 实例池状态筛选可用，与独享页一致 | ⬜ | | 筛选/搜索叠加走查 |
| 10 | U3 | 轮询间隔可配 + 独享页自动轮询 + 折叠面板 | ⬜ | | 配置生效 + kill 进程状态自动更新 |
| 11 | U4 | 日志分页：5000+ 条流畅 | ⬜ | | 翻页走查 |
| 12 | U5 | 统计迷你图（纯 CSS） | ⬜ | | build + 目视 |
