...
```
并行主链 A：M3~M6（验证/CI，随时穿插）→ S1（竞速预算）→ S5（自适应竞速）
并行支线 B：E1（上游代理出口）——【优先，与 A 同时开工】
并行支线 D：S4（日志补齐）——独立，可并行
主链后并行：S2（429 感知）→ S3（质量自愈）【依赖 S1 完成】
```

> 依赖说明：**S5 必须排在 S1 之后**（S5 动态副本需要 S1 的预算机制兜底）；S2/S3 依赖 S1
> （同文件），主链完成后并行；**E1 优先且独立**（客户最急的对症解药，不阻塞 S 系列）。
...
| 执行序 | 阶段 | 名称 | 状态 | 并行性 |
|---|---|---|---|---|
| 1 | M3~M6 | 多端收尾验证（CI 触发/真机） | 🔶 配置完成，待验证 | 可穿插 |
| 2 | **E1** | 上游代理出口（socks5 直连 Clash，MVP） | ⬜ 未开工 | ⚡ 优先独立，与 S1 并行 |
| 3 | **S1** | 竞速整体预算 + 快速失败 + 冷启动不竞速 | ⬜ 未开工 | 主链 A |
| 4 | **S5** | 自适应竞速：压力系数动态副本 + in-flight 均衡 | ⬜ 未开工 | 依赖 S1 |
| 5 | **S4** | 调用日志补齐 + 429 提示 + 探活目标可配 | ⬜ 未开工 | 独立，可并行 |
| 6 | **S2** | 429 感知：降级单发 + 指数退避 + 可见报错 | ⬜ 未开工 | 依赖 S1，主链后 |
| 7 | **S3** | 质量自愈：分类型恢复 + 半开竞速 + unknown 等级 | ⬜ 未开工 | 依赖 S1，主链后 |
