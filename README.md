...
- **Headless（桌面壳无头模式）**：安装桌面版后也可用 `opencode2api --headless [-port 40000] [-listen 127.0.0.1]` 启动——Rust 壳释放内嵌组件并拉起 core，无窗口运行，适合 SSH / 无图形会话；退出时自动清理网关与实例进程，详见 [Linux 开发与构建指南](docx/LINUX-DEVELOPMENT.md)。
