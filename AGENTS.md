...
- **UI 纪律（⭐ 全平台唯一界面）**：**六页 UI（独享 / 实例池 / 节点池 / 统计 / 日志 / 设置）是
  本项目全平台唯一事实界面**。不管哪个终端设备（Win exe / Web 浏览器 / 未来的 macOS / Linux），
  不管用什么语言或技术栈开发客户端，界面都必须与 exe 这六个菜单**一致**——复用 `src/` 同一套
  前端，或按该 UI 逐一对应实现；**禁止另起一套不同风格的界面**（历史上有过 `feature/web-self-service`
  分支另做简单页面，已废弃，不要效仿）。新增页面/菜单需先与这六页对齐，改动六页布局需显式确认。
- **提交纪律**：一阶段一提交，测绿才推进。
